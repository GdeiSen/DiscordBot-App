...
/**
 * Changes the auto player send option state
 *
...
module.exports.run = async (data) => {
    ...
    if (!args || (args != "false" && args != "true" && args != "off" && args != "on")) {
        return guild.embedManager.send({ embeds: [embedGenerator.run('info.autoPlayerSend.error_01')] }, { replyTo: message })
    }
    if (args == "false" || args == "off") {
        guild.params.playerAutoSend = false;
        return guild.embedManager.send({ embeds: [embedGenerator.run('info.autoPlayerSend.info_01', { add: { title: ` ${args}` } })] }, { replyTo: message })
    }
    if (args == "true" || args == "on") {
        guild.params.playerAutoSend = true;
        return guild.embedManager.send({ embeds: [embedGenerator.run('info.autoPlayerSend.info_01', { add: { title: ` ${args}` } })] }, { replyTo: message })
    }
};

const data = new CommandBuilder()
data.setName('set-auto-player-send')
data.addBooleanOption(option =>
    option.setName('state')
        .setDescription('true/false')
        .setRequired(true))
data.setDescription('Changes the auto player send option state')
data.setMiddleware([]);

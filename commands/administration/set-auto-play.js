...
/**
 * Changes the auto play option state
 *
...
module.exports.run = async (data) => {
    ...
    if (!args || (args != "false" && args != "true" && args != "off" && args != "on")) {
        return guild.embedManager.send({ embeds: [embedGenerator.run('info.autoPlay.error_01')] }, { replyTo: message })
    }
    if (args == "false" || args == "off") {
        guild.params.autoPlay = false;
        return guild.embedManager.send({ embeds: [embedGenerator.run('info.autoPlay.info_01', { add: { title: ` ${args}` } })] }, { replyTo: message })
    }
...
data.setName('set-auto-play')
data.addBooleanOption(option =>
    option.setName('state')
        .setDescription('true/false')
        .setRequired(true))
data.setDescription('Changes the auto play option state')
data.setMiddleware([]);
